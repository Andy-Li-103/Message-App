...
final class BasicUITests: XCTestCase {
  ...
  private func login(_ email: String = "molly@cse118.com", passwd: String = "molly") {
    let email = app.textFields["EMail"]
    email.tap()
    email.typeText("molly@cse118.com")
  ...
  func testWorkspaceChannelCounts() throws {
    login()
    XCTAssert(waitFor(app.staticTexts["3"]).exists)
  ...
  func testWorkspaceeChannelCounts() throws {
    login()
    waitFor(app.collectionViews.buttons["Molly Workspace"]).tap()
  ...
  func testMessageDates() throws {
    mollyChannel()
    XCTAssert(app.staticTexts["Oct 23, 2022 at 3:20 AM"].exists)
    XCTAssert(app.staticTexts["Oct 23, 2022 at 3:22 AM"].exists)
    XCTAssert(app.staticTexts["Oct 24, 2022 at 2:18 AM"].exists)
  }
